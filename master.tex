...
\section{Data}
\label{sec:data}
We use data from different runs. The following runs were made particular
for this work: (see Table~\ref{tab:results}). Other spectra were found in
the archives. We obtain the spectra with the highest possible resolution
for a given spectrograph, and in cases the are multiple observations, we
include all unless the spectra is close to the saturation limits for a
given spectrograph. For multiple spectra, we combine them by first correcting
the radial velocity (RV) and using a sigma clipper to remove cosmic rays.
The individual spectra are then combined to a single spectrum for a given
star which is used in the analysis below, in order to increase the SNR.

We have obtained spectra from UVES, FEROS, HARPS, FIES, and ESPaDONs.
...
\begin{enumerate}
    ...
    \item For each iteration save the parameters used and the quadratic sum of
          ...
          found parameters.
\end{enumerate}
This whole process is schamtically shown in Figure~\ref{fig:MOOGme_minimization}.
The steps follows these simple equations:
\begin{align}
...
form for changing the parameters are found by trial and error. We are looking
...
this is to make sure we do not take the logarithm to 0. After having a similar
function for $\log g$, we see that it could change to rapidly and we will
get unphysical request for parameters with high $T_\mathrm{eff}$ and low
