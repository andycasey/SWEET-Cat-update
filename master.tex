...
parameters from the 582 sample by \citet{Sousa2011}. Their procedure
resemble the one we choose for MOOGme. \citet{Sousa2011} used ARES
(the first version) for measuring the EWs and a minimization routine
based on the downhill simplex method. Moreover an older version of MOOG (WHICH)
were used for calculating the abundances.
...
\begin{table}[htb!]
    ...
    \begin{tabular}{lll}
      ...
      \hline
      $T_\mathrm{eff}$      &  $50   \pm 5$     & $50   \pm 5$                     \\
      $\log g$              &  $0.01 \pm 0.01$  & $0.01 \pm 0.01$                  \\
      $[\ion{Fe}/\ion{H}]$  &  $0.01 \pm 0.01$  & $0.01 \pm 0.01$                  \\
      $\xi_\mathrm{micro}$  &  $0.01 \pm 0.01$  & $0.01 \pm 0.01$                  \\
      \hline
    \end{tabular}
\end{table}

These offset can be due to a number of things. 1. the interpolation is
slightly different for the two methods, but produce very similar
results. 2. The different version of MOOG can lead to small changes
in abundances, and hence the parameters. 3. While we do use different
versions of ARES, the main difference here is how we set the rejt parameter.
A different rejt paramater can lead to a systematically offset in
[\ion{Fe}/\ion{H}]. Therefore, we applied the minimization of MOOGme
directly on the line lists from \citet{Sousa2011}, in order to have
identical EWs for all lines. The results are very similar, and the mean
difference is presented in the last column of Table~\ref{tab:MOOGmeTest}.
