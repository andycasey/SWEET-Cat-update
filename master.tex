...
photometric transit and radial velocity methods. Especially the latest announcement
...
three different modes: Measure EWs with ARES, EW method,
...
\label{sub:EW_method}
The EW method is a standard method for obtaining parameters from stellar
spectra. Here measured EWs are used to calculate abundances using a
...
\begin{align}
    f(\{T_\mathrm{eff}, \log g, [Fe/H], \xi_\mathrm{micro}\}) &= \sqrt{a_\mathrm{EP}^2 + a_\mathrm{RW}^2 + \Delta\ion{Fe}^2 + \ion{Fe}{I}^2} \\
                                                              &= z
\end{align}
...
\begin{itemize}
    ...
    \item \emph{refine}: After the minimization is done, run it again from the best
          ...
          it will always be the last step (after removal of outliers).
\end{itemize}
...
does not work well for cool stars. This
was fixed in \citet{Tsantaki2013} by removing some lines from \citet{Sousa2008a}.
...
line lists, interpolation of atmosphere grid, and minimization routine. We therefore randomly
...
NOTE: This section is probably not necessary...
